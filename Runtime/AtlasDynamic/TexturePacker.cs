...
public class TexturePacker
{
	...
	static public Rect[] PackTextures(Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize)
	{
		...
		{
			Debug.LogError("PackTextures than max size, return null");
			return null;
		}

		if (width > maxSize || height > maxSize) { int temp = width; width = height; height = temp; }

		// Force square by sizing up  NGUISettings.forceSquareAtlas
		if (TexturePacker.forceSquareAtlas)
		{
			if (width > height)
				height = width;
			...
			for (xPadding = 1; xPadding >= 0; --xPadding)
			{
				for (yPadding = 1; yPadding >= 0; --yPadding)
				{
					rect = bp.Insert(tex.width + (xPadding * padding), tex.height + (yPadding * padding),
						TexturePacker.FreeRectChoiceHeuristic.RectBestAreaFit);
	...
	static public Rect[] PackTextures(out RectInfo texture, RectInfo[] textures, int width, int height, int padding, int maxSize = 4096)
	{
		if (width > maxSize && height > maxSize)
		{
			texture = new RectInfo { width = width, height = height };
			Debug.LogError("width:"+ width+ " height:"+ height+">"+ maxSize);
			return null;
		}
		if (width > maxSize || height > maxSize) { int temp = width; width = height; height = temp; }
		...
		for (int i = 0; i < textures.Length; i++)
		{
			...
			for (xPadding = 1; xPadding >= 0; --xPadding)
			{
				...
				{
					rect = bp.Insert(tex.width + (xPadding * padding), tex.height + (yPadding * padding),
						TexturePacker.FreeRectChoiceHeuristic.RectBottomLeftRule);
					if (rect.width != 0 && rect.height != 0) break;

					// After having no padding if it still doesn't fit -- increase texture size.
					else if (xPadding == 0 && yPadding == 0)
					{
						return PackTextures(out texture, textures, width * (width <= height ? 2 : 1),
							height * (height < width ? 2 : 1), padding, maxSize);
					}
				}
				if (rect.width != 0 && rect.height != 0) break;
